...
fn main() -> xr::Result<()> {
    ...
    let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor{
        ...
        multiview: (views.len() > 1).then(|| (views.len() as u32).try_into().ok().unwrap()),
    });
    ...
    let mut swapchain = session.create_swapchain(&xr::SwapchainCreateInfo{
        ...
        width, height,
        face_count: 1,
        array_size: views.len() as u32,
        mip_count: 1,
    })?;
    let images = swapchain.enumerate_images()?.into_iter().map(|image| {
        let desc = wgpu::TextureDescriptor {label: None, size: wgpu::Extent3d{width, height, depth_or_array_layers: views.len() as u32}, mip_level_count: 1, sample_count: 1, dimension: wgpu::TextureDimension::D2, format, usage: wgpu::TextureUsages::RENDER_ATTACHMENT|wgpu::TextureUsages::TEXTURE_BINDING};
    ...
    loop {
        ...
        let ref view = images[index].create_view(&wgpu::TextureViewDescriptor{base_array_layer: 0, array_layer_count: (views.len() as u32).try_into().ok(), ..Default::default()});
