    ...
    struct ImplementOpenXrProgram : sample::IOpenXrProgram {
        ...
        void Run() override {
            if (!m_options.isStandalone) {
                printf("EnableRemotingXR\n");
                m_usingRemotingRuntime = EnableRemotingXR();

                if (m_usingRemotingRuntime) {
                    if (m_options.secureConnection) {
                        printf("InitializeSecureConnection\n");
                        m_secureConnectionCallbacks.InitializeSecureConnection();
                    }
                } else {
                    printf("RemotingXR runtime not available. Running with default OpenXR runtime.\n");
                }
            }

            printf("CreateInstance\n");
            CreateInstance();
            printf("CreateActions\n");
            CreateActions();

            printf("InitializeSystem\n");
            InitializeSystem();
            printf("InitializeDevice\n");
            InitializeDevice();

            printf("CreateWindowWin32\n");
            CreateWindowWin32();
            ...
            *exitRenderLoop = *requestRestart = false;
            ...
            while (pollEvent(eventData)) {
                ...
                case XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED: {
                    ...
                    switch ((XrSessionState)m_sessionState) {
                    ...
                    case XR_SESSION_STATE_MAX_ENUM:
                        DEBUG_PRINT("Ignoring session state %d", m_sessionState);
                        break;
                    }
                    break;
                }
